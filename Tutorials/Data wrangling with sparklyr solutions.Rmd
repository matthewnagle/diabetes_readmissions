...
sleep = copy_to(sc, msleep, overwrite = TRUE)
```
...
sleep %>% 
  ...
  group_by(order) %>%
  summarise(mean_brain_body_wt_ratio = mean(brain_body_wt_ratio, na.rm = TRUE)) %>%
  kable()
```
...
sleep = sleep %>%
  mutate(
    brainwt_imputed = case_when(
      is.na(brainwt) ~ mean(brainwt, na.rm = TRUE),
      TRUE ~ brainwt)) 

# Alternatively
# sleep = sleep %>%
#   mutate(
#     brainwt_imputed = ifelse(is.na(brainwt), mean(brainwt), brainwt))
...
ggplot(sleep_group, aes(x = mean_sleep_total, y = order)) + 
  geom_col(fill = "SteelBlue") + 
  ylab('Order') +
  xlab('Mean sleep hours')
```
