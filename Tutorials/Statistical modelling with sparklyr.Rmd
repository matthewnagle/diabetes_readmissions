...
```{r}
#method to one-hot encode quickly
cars <- cars %>%
  mutate(cyl = as.character(cyl)) %>%
  ft_string_indexer('cyl', 'cyl_indexed') %>%
  ft_one_hot_encoder_estimator('cyl_indexed', 'cyl_encoded') %>%
  ft_vector_assembler(c('hp', 'wt', 'cyl_encoded'), 'features')

#resultant object is not accepted by tidy()
ols_model <- ml_linear_regression(cars, features_col = 'features', label_col = 'mpg', fit_intercept = FALSE)
...
```{r}
ols_model <- ml_linear_regression(
  cars, mpg ~ wt + hp + cyl_4 + cyl_6 + cyl_8, fit_intercept = FALSE)
...
ols_model_log <- ml_linear_regression(
  cars, log_mpg ~ wt + hp + cyl_4 + cyl_6 + cyl_8, fit_intercept = FALSE)
...
```{r}
ols_eval <- ml_evaluate(ols_model_log, cars)

ols_eval$r2
ols_eval$mean_absolute_error
ols_eval$mean_squared_error
ols_eval$root_mean_squared_error
...
```{r}
predictions = ml_predict(ols_model_log)

mutate(predictions,
  squared_error = (log_mpg - prediction)^2
) %>%
